...
void n_box(int x, int y, int w, int h, int t_color, int b_color, char nd[][30], int sl) 
{
	...
	for (int i = 0; i < sl; i++)
	{
		...
		if (i != 0)
		{
			gotoXY(x, y + (i * h)); cout << char(179);
			gotoXY(x + w, y + (i * h)); cout << char(179);
		}
	}
}
...
void menu1()
{
	...
	int h = 3;
	int t_color = 2;
	int b_color = 5; // hinh nen mau den
	int b_color_sang = 75; // 75 la hinh nen mau do 
	int sl = 4;
...
void menu_chinh()
{
	int x = 40;
	int y = 5;
	int w = 22; // chieu dai cua khung 
	int h = 3; // do cao cua khung 
	int t_color = 2;
	int b_color = 5;
	int b_color_sang = 75;

	int sl = 2;
	char nd1[50] = { "----PBL2: DU AN SO SO LAP TRINH----" };
	box(35, 0, 36, 2, 11, 10, nd1);
	char nd2[50] = { "--DE TAI: XAY DUNG HE THONG QUAN LY BAN HANG--" };
	box(28, 2, 50, 2, 11, 10, nd2);
	char nd3[200] = { "  SINH VIEN THUC HIEN: TRAN VAN NGUYEN - NGUYEN THI LE\n\t\t\t\tLOP HOC PHAN: 21TCLC-DT2 - NHOM 21.12A\n\t\t\t\t  GIANG VIEN HUONG DAN: MAI VAN HA" };
	//n_box(35, 5, 22, 3, 2, 5, nd3,2);
	box(24, 12, 56, 5, 11, 10, nd3);
	...
	while (true)
	{
		...
		if (kt == true)
		{
			//----- back space ----
			gotoXY(xcu, ycu);
			thanh_sang(xcu, ycu, w, h, b_color, nd_chinh);//rs thanh sang cu // di chuyen thanh sang mau do 
			xcu = xp; ycu = yp;
			//-------------
			thanh_sang(xp, yp, w, h, b_color_sang, nd_chinh);// bắt màu thanh sang mau do 
			kt = false;
		}
		//----- dieu khien ---- //----- di chuyen ----
		if (_kbhit())
...
string getPass()
{
	string password;
	char ch;
	while ((ch = _getch()) != 13) // bat phim enter de dung lai 
	{
		if (ch == 8)
		{
...
void menu_ctdh()
{
	...
	gotoXY(90, 13); cout << "Discount: " << (sum- p.getTien(mdh))*100/sum << " %";
